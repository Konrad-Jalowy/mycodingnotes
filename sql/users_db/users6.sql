...
(1, 250.00, '2024-02-01'),  -- Jan Kowalski
(1, 100.00, '2024-01-15'),  -- Jan Kowalski
(2, 300.00, '2024-03-10'),  -- Anna Nowak
(3, 150.00, '2024-02-20'),  -- Piotr Zieliński
(3, 500.00, '2024-02-25');  -- Piotr Zieliński
...
-- Pobranie użytkowników, którzy wydali cokolwiek, ale mniej niż 350
SELECT users.first_name, users.last_name, SUM(orders.total_price) AS total_spent
FROM users
INNER JOIN orders ON users.id = orders.user_id
GROUP BY users.id, users.first_name, users.last_name
HAVING SUM(orders.total_price) < 350;
...
FROM users
JOIN orders ON users.id = orders.user_id
WHERE orders.created_at BETWEEN '2024-02-01' AND '2024-02-28'
