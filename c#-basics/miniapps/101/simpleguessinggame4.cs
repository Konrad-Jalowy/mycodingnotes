namespace ConsoleApp18
{
    ...
    class Game
    {
        ...
        private void PlayRound()
        {
            ...
            {
                Console.Write($"Pozostało prób: {MaxAttempts - attempts}. Podaj swoją liczbę (lub wpisz '!zmien' aby zmienić limit prób): ");
        ...
        private void ChangeMaxAttempts()
        {
            Console.Write("Podaj nowy limit prób: ");
            if (int.TryParse(Console.ReadLine(), out int newLimit) && newLimit > 0)
            {
                if (newLimit < MaxAttempts)
                {
                    Console.WriteLine($"Nowy limit prób to {newLimit}. Bądź ostrożny!");
                }
                else
                {
                    Console.WriteLine($"Nowy limit prób to {newLimit}. Masz więcej czasu na odgadnięcie!");
                }
                MaxAttempts = newLimit;
            }
            else
            {
                Console.WriteLine("Nieprawidłowy limit prób. Pozostaje poprzednia wartość.");
            }
        }
