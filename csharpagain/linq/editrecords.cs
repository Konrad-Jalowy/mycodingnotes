...
class Program
{
    ...
    static void Main()
    {
        ...
        // Interaktywna edycja rekordów
        while (true)
        {
            Console.WriteLine("Czy chcesz edytować osobę? (tak/nie)");
            string input = Console.ReadLine()?.Trim().ToLower();
            if (input != "tak") break;
            ...
            var personToEdit = people.FirstOrDefault(p => p.Id == id);
            if (personToEdit != null)
            {
                ...
                Console.Write("Podaj nowy wiek (pozostaw puste, aby nie zmieniać): ");
                string newAge = Console.ReadLine();
                if (!string.IsNullOrWhiteSpace(newAge))
                    personToEdit.Age = int.Parse(newAge);
