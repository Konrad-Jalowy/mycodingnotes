...
def menu():
    while True:
        ...
        if choice == "1":
            ...
            title = input("Nowy tytuł (ENTER = brak zmian): ") or None
            description = input("Nowy opis (ENTER = brak zmian): ") or None
            status = input("Nowy status (`pending`, `in progress`, `completed`): ") or None
            task_service.update_task(task_id, title, description, status)
        elif choice == "8":
