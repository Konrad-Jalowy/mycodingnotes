...
def login():
    ...
    # Dodajemy CSRF Token do odpowiedzi (widoczny dla JS)
    response.set_cookie("csrf_refresh_token", get_csrf_token(refresh_token), httponly=False)

    return response


@auth_bp.route("/refresh", methods=["POST"])
@jwt_required(refresh=True)  # Wymaga Refresh Tokena w Cookies
def refresh():
    """Odświeżanie Access Tokena za pomocą Refresh Tokena"""
    csrf_token = request.headers.get("X-CSRF-TOKEN")  # Pobieramy CSRF Token z nagłówka
